...
=======
  main = "Crime Density by LAPD Division (2010–2019)",
>>>>>>> refs/remotes/origin/main
...
=======
  main = "Crime Density by LAPD Division (2020–Present)",
>>>>>>> refs/remotes/origin/main
...
The plot analyzes the number of crimes over time using time series. The time series shows stable weekly crime counts with occasional large spikes. These surges indicate certain weeks where crime reporting jumped significantly, there is a recent interesting surge that seems to mean-revert in 2024.

## Crime rate frequency by day of the week

```{r, echo=FALSE, fig.cap = "Crime Occurrences by Day of the Week", warning=FALSE, message=FALSE}
>>>>>>> refs/remotes/origin/main
...
barplot(crime_counts, col = "lightblue",
        ...
        names.arg = names(crime_counts), las = 1, cex.names = 0.4, cex.axis = 0.6)
```

### Interpretation

The plot analyzes the crime rate frequency frequency by day of the week using a bar graph. The number of crimes are rather evenly distributed, with the weekends having slightly lower crime compared to week days, which may be a result from more people around during weekends which indirectly suprevises people's behavior. 
...
The pie chart analyzes the gender distribution of crime activities. From the chart, we can infer that around 50% of the crime is commited by male, 45% by female and 5% by non-binary. The crime distribution by gender is pretty even

## Median Household Income by Year and Zip Code in Los Angeles
...
zip_codes <- read.csv("0_Data/Cleaned_Data/sampled_crimes.csv")
...
## Monthly Frequency of Evictions and Crimes in the City of Los Angeles, February 2023 to February 2025
...
This histogram plots the frequency of evictions in Los Angeles from February 2023 to February 2025, where the City of Los Angeles Housing Department has published case data. By summarizing both the crime and the evictions datasets by month, we see that the number of evictions was highest in early-mid 2023, and since, there have typically been slightly lower frequencies of evictions. Additionally, we split the number of crimes per month based on the average number of crimes of 14,439 to denote above and below average number of crimes per month. There is a clear split between March and April of 2024, as all months before that had above average crime, and all after had below average. Interestingly, the months where there is below average crime, seem to be the ones with slightly lower numbers of evictions.

## Annual Number of Crimes by Los Angeles Police Department Bureau
...
This boxplot shows the number of crimes in the four different bureaus of Los Angeles annually. We categorized the areas into their respective Central, South, Valley, and West Bureaus. The South Bureau has the highest median number of crimes, as well as the largest variability, while the Central, Valley, and West bureaus have relatively similar distributions with lower crime counts.The outliers suggests some years had significantly lower crime counts than others. This visualization helps us understand how crime is distributed across different regions, with the South experiencing the highest fluctuation and overall crime levels.

## Correlations Between the Number of Crimes and Different Law Enforcement and Socioeconomic Factors
...
This heat map shows the correlations between different variables that could affect the number of crimes committed. We see that the most significant correlations for the number of crimes are the unemployment rate, year, and number of evictions. Years is positively correlated with number of crimes, meaning that as time passed, the number of crimes increased. Similarly, as the number of evictions increases, so does the number of crimes. Interestingly, as the unemployment rate increases, the number of crimes decreases, which is the opposite effect of what was expected (one would anticipate that higher unemployment would result in more crime). The LAPD budget interestingly had a medium-strength positive correlation with number of crimes, suggesting we should further examine the increase in the LAPD budget compared to the increase in crimes.

## Homicide Victims by Age and Gender in Los Angeles 2010-Present
...
This jitter plot shows us both the frequencies and the demographics of homicide victims in Los Angeles from 2010 to present. Based on the density of the points, we see that most homicide victims in the last 15 years have been men between the ages of around 15 to 65. Additionally, we see that there are not many victims who are toddlers and younger children, but there is a good number of babies who have been killed. This is an intricacy in our data that might be interesting to look into further, particularly into the circumstances of those deaths and potential correlations between reports of child abuse and/or domestic violence and infanticide.

## Frequency of Crimes in Los Angeles Throughout the Day
...
This density plot shows the frequency of crimes depending on the hour (in military time) that they occurred. Based on the plot, we see that fewer crimes occur early in the morning (with the lowest at 5 AM). The data peaks at noon, meaning the most crimes occur in that hour of the day. Crimes are more frequent after that, until the early morning. The second most common crime occurrence is 6 PM, or 18 hours.

## Time series analyzing homicide rate trends over time by socioeconomic quartiles
...
=======
    fill = "Income Quartile",
    title = "Homicide Rates in Los Angeles By Median Income Level"
>>>>>>> refs/remotes/origin/main
  ) +
  theme_minimal() +
...
Homicide rates were low pre-pandemic and had a sharp increase since the pandemic, except in areas of low income, where homicide rates were high before the pandemic. In lower income areas, the rates of homicide have decreased significantly since the end of the pandemic. On the other hand, areas of higher-middle and high income have seen large increases in homicide in the six years shown.

[BASED ON THESE PLOTS, WE WILL DO XYZ TO EXAMINE ABC]
...
## Regression Between Median Income and Crimes per Zip Code - DROP THIS
...
## Model Progression
### Initial Spatial Analysis of Crime in Los Angeles

```{r, echo=FALSE, warning=FALSE, message=FALSE}
homicides_latlon_rounded <- homicides %>% mutate(LAT_rounded = round(LAT, 1),
                                                 LON_rounded = round(LON, 1))
homicides_lat_long <- homicides_latlon_rounded %>% group_by(LAT_rounded, LON_rounded) %>% summarize(HomicideCount = n()) %>% ungroup()

linear_model1 <- lm(HomicideCount ~ LAT_rounded + LON_rounded, data = homicides_lat_long)

summary(linear_model1) # longitude rounded is significant, LAT_rounded is not
```
...
linear_model2 <- lm(HomicideCount ~ LAT_rounded + LON_rounded + year, data = homicides_by_year_lat_long)

summary(linear_model2) # longitude rounded is significant, LAT_rounded is not. Neither is year
```
...
median_household_income <- cali_median_income %>% dplyr::select(ZCTA, S1903_C03_001E, year)
=======
median_household_income <- cali_median_income %>% dplyr::select(ZCTA5CE20, S1903_C03_001E, year)
>>>>>>> refs/remotes/origin/main
...
#zip_codes <- reverse_geocode(sampled_ids, lat = LAT, long = LON, verbose = TRUE)
#write.csv(zip_codes, "sampled_crimes.csv")
zip_codes <- read.csv("0_Data/sampled_crimes.csv")
...
####Figure 7 - Monthly Frequency of Evictions and Crimes in the City of Los Angeles, February 2023 to February 2025 {.unnumbered}
...
=======
#### Figure 8 - Annual Number of Crimes by Los Angeles Police Department Bureau {.unnumbered}
...
=======
#### Figure 9 - Correlations Between the Number of Crimes and Different Law Enforcement and Socioeconomic Factors {.unnumbered}
...
correlations_df <- crime_la_monthly %>% inner_join(unemployment, join_by("year" == "Year", "month" == "month")) %>% dplyr::select(month, year, Value, num_crimes) %>% rename("unemployment_rate" = "Value")
=======
correlations_df <- crime_la_monthly %>% inner_join(unemployment, join_by("year" == "Year", "month" == "month")) %>% select(month, year, Value, num_crimes) %>% rename("unemployment_rate" = "Value")
>>>>>>> refs/remotes/origin/main
...
correlations <- correlations_df4 %>% dplyr::select(year, month, unemployment_rate, num_crimes, cases, num_evictions, police_budget)
=======
correlations <- correlations_df4 %>% select(year, month, unemployment_rate, num_crimes, cases, num_evictions, police_budget)
>>>>>>> refs/remotes/origin/main
...
homicide_by_gender <- crime_la %>% filter(Crm.Cd.Desc == "CRIMINAL HOMICIDE") %>% dplyr::select(Vict.Sex, DateParsed, Vict.Age) %>% mutate(gender = factor(Vict.Sex))
=======
#### Figure 10 - Homicide Victims by Age and Gender in Los Angeles 2010-Present {.unnumbered}

```{r, eval=FALSE}
homicide_by_gender <- crime_la %>% filter(Crm.Cd.Desc == "CRIMINAL HOMICIDE") %>% select(Vict.Sex, DateParsed, Vict.Age) %>% mutate(gender = factor(Vict.Sex))
>>>>>>> refs/remotes/origin/main
...
=======
#### Figure 11 - Frequency of Crimes in Los Angeles Throughout the Day {.unnumbered}
...
homicides <- crime_la %>% inner_join(crimes_with_zips, by = c("DR_NO", "LAT", "LON","X", "year")) %>% filter(Crm.Cd.Desc == "CRIMINAL HOMICIDE") %>% 
  mutate(date = as.Date(DateParsed),
         zip = as.character(ZCTA5CE20)) %>% dplyr::select(-X)
=======
#### Figure 12 - Time series analyzing homicide rate trends over time by socioeconomic quartiles {.unnumbered}

```{r, eval=FALSE}
# Data Preparation
homicides <- crime_la %>% inner_join(crimes_with_zips, by = c("DR_NO", "LAT", "LON","X", "year")) %>% filter(Crm.Cd.Desc == "CRIMINAL HOMICIDE") %>% 
  mutate(date = as.Date(DateParsed),
         zip = as.character(ZCTA5CE20)) %>% select(-X)
>>>>>>> refs/remotes/origin/main
