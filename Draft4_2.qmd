...
correlations_df <- crime_la_monthly %>% inner_join(unemployment_lapd_budget_evictions, join_by("year" == "Year", "month" == "month")) %>% dplyr::select(month, year, num_evictions, unemployment_rate, num_crimes, police_budget, Crm.Cd.Desc)
...
```{r  fig.cap="Crime densities in Los Angeles by LAPD Division, message=FALSE, warning=FALSE, 2010.2019 and 2020.Present", fig.align="center"}
query <- dbSendQuery(con, "Select * From crime_la Where year < 2020")
crime_la_2010_2019 <- dbFetch(query,n=-1)
dbClearResult(query)
...
march_2020_index <- which(weekly_counts$YearWeek == "2020-09")  # Adjust '202009' to match your data's YearWeek format for March 2020
...
correlations_df <- crime_la_monthly %>% inner_join(unemployment_lapd_budget, join_by("year" == "Year", "month" == "month")) %>% dplyr::select(month, year, Value, num_crimes, police_budget) %>% rename("unemployment_rate" = "Value")
...
correlations <- correlations_df3 %>% dplyr::select(year, month, unemployment_rate, num_crimes, cases, num_evictions, police_budget)
