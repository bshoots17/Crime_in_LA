...
ggplot(data = evictions_la_indexed_crimes, aes(x = index)) + 
  ...
  theme(axis.text.x = element_text(angle = 300, vjust = 0.5, hjust = -.1),
...
# covid data
covid_14day$date <- ymd(covid_la$ep_date)
covid_14day$month <- month(covid_la$date)
covid_14day$year <- year(covid_la$date)
covid_14day$date <- day(covid_la$date)
# since there is a 7 day lag in reporting, let's do monthly = cases on the 21st and on the 5th of the next month.
covid_14day$lagged <- covid_la$date + 7
...
# base plot shows that this does in fact mirror LA county so the case numbers should be accurate
# plot(y = covid_la_monthly$cases, x = covid_la_monthly$index, type = "l")
