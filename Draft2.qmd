...
homicides <- crime_la %>%
  inner_join(crimes_with_zips, by = "crime_id") %>%  
  filter(offense_type == "HOMICIDE") %>%
  mutate(date = as.Date(date)) 

# Join homicide data with income data
homicides <- homicides %>%
  left_join(cali_median_income, by = c("zip_code" = "ZCTA"))

# Convert median income to numeric if needed
homicides$median_income <- as.numeric(homicides$median_income)

# Create income quartiles
homicides <- homicides %>%
  mutate(income_quartile = ntile(median_income, 4))  # Divides into 4 quartiles

# Aggregate homicide counts by year and income quartile
homicide_trends <- homicides %>%
  mutate(year = lubridate::year(date)) %>%
  group_by(year, income_quartile) %>%
  summarize(homicide_count = n(), .groups = "drop")
...
ggplot(homicide_trends, aes(x = year, y = homicide_count, color = factor(income_quartile))) +
  ...
  labs(
    title = "Homicide Rate Trends Over Time by Socioeconomic Quartiles",
    x = "Year",
