...
evictions_la <- read.csv("Raw_Data/LA_evictions.csv")
```
...
## Number of Evictions and Crime Levels Per Month
```{r, echo=FALSE}
# fix mistake
...
ggplot(data = evictions_la_indexed_crimes, aes(x = index)) + 
  ...
  labs(x = "Month and Year of Eviction Notice Receipt", y = "Number of Evictions", title = "Monthly Frequency of Evictions and Crimes in the City of Los Angeles \nFebruary 2023 to February 2025", fill = "Crime Level") +
...
This boxplot shows the number of crimes in different bureaus of Los Angeles. We categorized them into Central, South, Valley, and West. The South bureau has the highest median number of crimes, as well as the largest variability, while the Central, Valley, and West bureaus have relatively similar distributions with lower crime counts.The outliers suggests some months had significantly lower crime counts than others. This visualization helps us understand how crime is distributed across different regions, with the South experiencing the highest fluctuation and overall crime levels.
...
LA_county_evictions <- read.csv("Raw_Data/LA_county_evictions.csv")
...
unemployment <- read.csv("Raw_Data/la_county_unemployment_rate.csv")
unemployment$month <- as.numeric(gsub("M", "", unemployment$Period))

# lapd budget
lapd_budget <- read.csv("Raw_Data/lapd_budget.csv")
...
ggplot(data = long_Cor, aes(x = Var1, y = Var2, fill = value)) +
  ...
  geom_text(aes(label = value), color = "black", size = 4) + 
   theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(size=12)) + 
  ggtitle("Correlations Between the Number of Crimes and \n Different Law Enforcement and Socioeconomic Factors")
```
...
```{r, echo=FALSE}
ggplot(crime_la, aes(x = DateParsed, y = Vict.Sex, color = Vict.Sex)) +
  geom_jitter(alpha = 0.4, width = 0.2) +
  labs(title = "Crime Over Time by Gender",
       x = "Date",
       y = "Gender") +
  theme_minimal()
...
crime_la$Hour <- as.numeric(format(crime_la$DateParsed, "%H"))

ggplot(crime_la, aes(x = Hour, fill = ..density..)) +
  geom_density(alpha = 0.5, color = "black") +
  scale_fill_gradient(low = "lightblue", high = "darkblue") +
  labs(title = "Crime Density Per Hour",
       x = "Hour",
       y = "Density") +
  theme_minimal()
...
```{r, echo=FALSE}
# Data Preparation
homicides <- crime_la %>% inner_join(crimes_with_zips, by = "crime_id") %>% filter(offense_type == "HOMICIDE") %>% mutate(date = as.Date(date)) 
homicides <- homicides %>% left_join(cali_median_income, by = c("zip_code" = "ZCTA"))
homicides$median_income <- as.numeric(homicides$median_income)

# Form income quartiles
homicides <- homicides %>% mutate(income_quartile = ntile(median_income, 4)) 
homicide_trends <- homicides %>% mutate(year = lubridate::year(date)) %>% group_by(year, income_quartile) %>% summarize(homicide_count = n(), .groups = "drop")

# Plot homicide trends over time by income quartile
ggplot(homicide_trends, aes(x = year, y = homicide_count, color = factor(income_quartile))) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  labs(
    x = "Year",
...
## Interpretation
Homicide rates were low pre-pandemic and had a sharp increase since pandemic due to a reduction in income. Homicide rate reduces again after pandemic likely due to the economic recovery and increasing wages. 
