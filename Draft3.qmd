...
```{r, echo=FALSE, fig.cap = "Crime Occurrences by Day of the Week", message=FALSE}
...
## Chi-Square for Zip Code

```{r}
homicides_by_zip <- homicides %>% group_by(ZCTA5CE20) %>% summarize(HomicideCount = n()) %>% ungroup()
chisq.test(x = as.character(homicides_by_zip$ZCTA5CE20), y = homicides_by_zip$HomicideCount)
```
