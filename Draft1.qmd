...
```{r}
# Calculate quantiles for the entire dataset (from 2018 to 2023)
quantiles <- round(quantile(zcta_no_NAs$S1903_C03_001E, probs = seq(0, 1, length.out = 7)), 0)

# Setting a layout for a 2x3 grid
...
for (yr in yrs) {
  ...
  # Divide into consistent bins based on the global quantiles
  bins <- cut(subset_data$S1903_C03_001E, breaks = quantiles, labels = FALSE, include.lowest = TRUE)
  ...
  # Add legend (use the global quantiles)
  legend("right", inset = c(-.75, 0), legend = quantiles, fill = palette, title = "Median Income ($)", bty = "n", cex = 0.8, xpd = TRUE)
}

# Outer title for the whole set of plots
