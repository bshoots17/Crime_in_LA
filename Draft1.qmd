...
crime_la_monthly <- crime_la %>% group_by(Year)
# convert to date
...
ggplot(data = evictions_la_indexed, aes(x = index)) + 
  geom_histogram(bins = nrow(unique_months)) +
  scale_x_continuous(breaks=unique_months$index,
        labels=unique_months$month_year) + 
  theme(axis.text.x = element_text(angle = 285, vjust = 0.5, hjust=1),
        axis.title.x = element_text(vjust = -.75),
        plot.title = element_text(size=12)) + 
  labs(x = "Month and Year of Eviction Notice Receipt", y = "Number of Evictions", title = "Monthly Frequency of Evictions in the City of Los Angeles \nFebruary 2023 to February 2025") + 
