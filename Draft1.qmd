...
# covid data
covid_la$date <- ymd(covid_la$ep_date)
covid_la$month <- month(covid_la$date)
covid_la$year <- year(covid_la$date)
covid_la$date <- day(covid_la$date)
# since there is a 7 day lag in reporting, let's do monthly = cases on the 21st and on the 5th of the next month.
covid_la$lagged <- covid_la$date + 7

covid_la_lagged <- covid_la %>% filter(lagged %in% c(21, 35)) %>% arrange(month, year)
